...
import { DataResponse, FileValidationModel, ModuleItemsList, ModuleList, UserResponseModel } from "./request-response/response";
...
@Injectable({
  ...
  export class ModuleItemsLookupUsecase {
    ...
    excute(): Observable<ModuleItemsList[]> {
      return this.httpClient.get<ModuleItemsList[]>(Configuration.APIs.LookupManagement.GetModuleItemList
        , { context: this.fileManagementAPIUrl })
        .pipe(map((res: ModuleItemsList[]) => {
            return res
          }))
        }
  }